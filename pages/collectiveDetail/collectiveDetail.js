...
Page({
  ...
  onShareAppMessage: function (options) {
    ...
    var path = '/pages/collectiveDetail/collectiveDetail?cid=' + page.data.cid + '&user_id=' + userInfo.id;
    return {
      title: page.data.goods.goods_name,
      path: path,
      imageUrl: page.data.goods.goods_picture,
  ...
  getInfo: function (e) {
    ...
    util.request(api.CollectiveDetailById, { id: cid}).then(function(res){
      if (res.errno == 0 ){
        page.countDownRun(res.data.limit_time_ms);
        var reduce_price = (res.data.goodsInfo.price - res.data.collectiveRule.goods_price).toFixed(2);
        page.setData({
          group_fail: res.data.collectiveInfo,
          goods: res.data.goodsInfo,//商品详情
          groupList: res.data.collectiveList,//开团人列表
          surplus: res.data.surplus,//剩余人数
          in_group: res.data.inGroup,
          goods_list: res.data.goodsList.data,
          group_rule: res.data.collectiveRule,//规则
          reduce_price: reduce_price < 0 ? 0 : reduce_price,//省
          goodsId: res.data.goodsInfo.goods_id
        })
      } else {
          wx.showModal({
            title: '提示',
            ...
            success: function (res) {
              if (res.confirm) {
                wx.redirectTo({
                  url: '/pages/ucenter/collective/collective'
                });
              }
            }
          });
...
*/
  ...
  goToGoodsDetails: function (e) {
    var page = this;
    wx.redirectTo({
      url: '/pages/collective/collective?id=' + page.data.goods.goods_id,
    });
  },
  /**
* 隐藏规格选择框
*/
  ...
  */
  ...
  //   wx.navigateTo({
  //     url: '/pages/collectiveOrder/collectiveOrder?goodsId=' + goodsId + '&num=' + page.data.form.number
  //   })
  // },
  submit: function (type) {
    ...
    wx.redirectTo({
      url: '/pages/collectiveOrder/collectiveOrder?goodsId=' + page.data.goodsId + '&num=' + page.data.form.number + '&collective_no=' + page.data.group_fail.collective_no,
    });
  },
